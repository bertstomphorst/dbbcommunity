...
class TestMessageNotification(MailMessageCommon):
    ...
    def setUp(self):
        ...
        MailMail.unlink = unlink_replacement
        ...
        self.message_dict = {
            ...
            "to": f"{self.res_partner_kate.name} <{self.res_partner_kate.email}>, {self.res_partner_ann.name} <{self.res_partner_ann.email}>",  # noqa: E501
            "cc": "",
            "references": "",
    ...
    def test_message_route_allow_direct_message(self):
        ...
        self.env["mail.alias.domain"].create(
            {"catchall_alias": "kate", "name": "example.com"}
        )

        # Default behavior
        # Allow direct message to catchall is false
        result = self.env["mail.thread"]._detect_write_to_catchall(self.message_dict)
